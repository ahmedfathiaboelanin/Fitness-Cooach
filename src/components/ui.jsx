...
export function Badge({ children, color = 'fire' }) {
  const map = {
    fire: 'bg-gradient-to-r from-amber-500 to-rose-600 text-white shadow-fire',
...
export function SectionTitle({ title, subtitle, center, eyebrow = '🔥 MOHAMMAD ELSUM' }) {
  return (
    <motion.div
      initial={{ opacity: 0, y: 18 }}
      whileInView={{ opacity: 1, y: 0 }}
      viewport={{ once: true, margin: '-60px' }}
      className={`${center ? 'text-center' : ''} mb-8`}
    >
      <span className="inline-flex items-center gap-2 text-[11px] font-extrabold tracking-[0.2em] text-rose-400 bg-rose-500/10 border border-rose-500/25 px-3 py-1.5 rounded-full">{eyebrow}</span>
...
export function RatingStars({ rating }) {
  return (
    <span className="flex items-center gap-1 text-sm">
      <span className="text-amber-400 animate-flicker">🔥</span>
      <b>{rating}</b>
    </span>
  )
}
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <motion.div initial={{ scale: 0.92, opacity: 0, y: 10 }} animate={{ scale: 1, opacity: 1, y: 0 }} className="relative bg-stone-900 border border-rose-500/20 rounded-2xl p-6 max-w-md w-full shadow-fire-lg">
        <button onClick={onClose} className="absolute top-4 right-4 text-stone-400 hover:text-white">✕</button>
        {children}
      </motion.div>
    </div>
  )
