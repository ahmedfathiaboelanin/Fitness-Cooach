...
import { Flame, GraduationCap, ArrowRight, Check } from 'lucide-react'
      ...
      <div className="relative container-x py-10 grid lg:grid-cols-[380px_1fr] gap-6">
        <div>
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="rounded-[1.8rem] p-1.5 bg-fire-gradient shadow-fire-lg">
            <img src={heroImg || coach.avatar} className="rounded-[1.5rem] w-full h-[460px] object-cover" alt={pick(coach, 'name')} />
          </motion.div>
          <Card className="mt-4 text-center">
            ...
            <WhatsAppButton className="w-full mt-4" text={isAR ? 'أهلاً محمد! عايز أتمرن معاك' : 'Hi Mohammad! I want to train with you'} label={isAR ? 'تواصل واتساب' : 'Contact on WhatsApp'} />
          </Card>
        </div>
        <div className="space-y-6">
          <div>
            <span className="text-[11px] font-extrabold tracking-[0.2em] text-rose-400 bg-rose-500/10 border border-rose-500/25 px-3 py-1.5 rounded-full inline-flex items-center gap-1.5"><Flame size={12} /> {t('myStory')}</span>
          ...
          <Card>
            <h3 className="font-display text-xl uppercase mb-3 flex items-center gap-2"><Flame size={18} className="text-rose-400" /> {isAR ? 'التخصصات' : 'Specializations'}</h3>
            <div className="flex flex-wrap gap-2">{specs.map((s) => <Badge key={s}>{s}</Badge>)}</div>
            <h3 className="font-display text-xl uppercase mt-6 mb-2 flex items-center gap-2"><GraduationCap size={18} className="text-rose-400" /> {isAR ? 'الشهادات' : 'Certifications'}</h3>
            <ul className="text-sm space-y-1.5 text-stone-300">{certs.map((c) => <li key={c} className="flex items-start gap-2"><Check size={15} className="text-rose-400 shrink-0 mt-1" /> {c}</li>)}</ul>
          </Card>
          <Card>
          ...
          <div className="flex flex-wrap gap-3">
            <Link to="/results" className="px-5 py-2.5 rounded-xl border border-white/15 bg-white/5 font-bold text-sm hover:border-rose-500/50 inline-flex items-center gap-1.5">{t('seeWork')} <ArrowRight size={15} className="rtl:rotate-180" /></Link>
            <Link to="/booking" className="btn-fire px-5 py-2.5 rounded-xl font-extrabold text-sm text-white inline-flex items-center gap-1.5">{t('bookPackage')} <ArrowRight size={15} className="rtl:rotate-180" /></Link>
          </div>
        </div>
      </div>
    </div>
