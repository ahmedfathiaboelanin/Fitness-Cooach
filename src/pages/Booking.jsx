  ...
  return (
    <div className="bg-stone-950 text-stone-100 min-h-screen">
      <div className="container-x py-10 max-w-3xl">
        <span className="text-[11px] font-extrabold tracking-[0.2em] text-rose-400 bg-rose-500/10 border border-rose-500/25 px-3 py-1.5 rounded-full">{t('bookCoaching')}</span>
            ...
            <div className="space-y-3">
              {packages.map((p) => (
                <motion.button key={p.id} whileHover={{ scale: 1.01 }} onClick={() => setPkgId(p.id)} className={`w-full flex justify-between items-center p-4 rounded-2xl border text-left transition ${pkgId === p.id ? 'border-rose-500 bg-rose-500/10 shadow-fire' : 'border-white/10 hover:border-rose-500/40'}`}>
                  <div><div className="font-bold text-white">{pick(p, 'name')} <span className="text-[10px] bg-fire-gradient text-white px-2 py-0.5 rounded-full ml-1">🔥 {pick(p, 'tag')}</span></div><div className="text-xs text-stone-400">{(p.features || []).slice(0, 2).join(' • ')}</div></div>
                  <div className="font-display text-xl text-fire">{p.price}</div>
                </motion.button>
              ))}
            </div>
            <Button className="mt-4 w-full" onClick={() => setStep(2)}>{isAR ? 'كمل ←' : 'Continue →'}</Button>
          </Card>
        )}
        {step === 2 && (
        ...
        {step === 3 && (
          <Card className="mt-6">
            <h3 className="font-display text-xl uppercase mb-3">{isAR ? '3. أكد على واتساب 💬' : '3. Confirm on WhatsApp 💬'}</h3>
            ...
            <div className="flex flex-col sm:flex-row gap-2 mt-4">
              ...
              <a href={waLink(message)} target="_blank" rel="noreferrer" className="flex-1 text-center bg-[#25D366] text-white font-extrabold px-5 py-2.5 rounded-xl shadow-[0_12px_35px_-10px_rgba(37,211,102,0.7)] hover:brightness-110">{isAR ? 'أكد على واتساب 🔥 ←' : 'Confirm via WhatsApp 🔥 →'}</a>
            </div>
