  ...
  return (
    <div className="bg-stone-950 text-stone-100 min-h-screen">
      <div className="container-x py-8">
        <Link to="/workouts" className="text-sm text-stone-400 hover:text-rose-300">{isAR ? '→ كل البرامج' : '← All programs'}</Link>
        <div className="grid lg:grid-cols-2 gap-6 mt-4">
          ...
          <Card>
            <div className="flex flex-wrap gap-2"><Badge color="green">{pick(w, 'level')}</Badge><Badge>{pick(w, 'goal')}</Badge><Badge color="fire">🔥 {pick(coach, 'name')}</Badge></div>
